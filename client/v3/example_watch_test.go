...
func ExampleWatcher_watch() {
	examples.forUnitTestsRunInMockedContext(mockWatcher_watch, func() {
		cli, err := New(Config{
...
func ExampleWatcher_watchWithPrefix() {
	examples.forUnitTestsRunInMockedContext(mockWatcher_watchWithPrefix, func() {
		cli, err := New(Config{
			Endpoints:   examples.exampleEndpoints(),
			DialTimeout: examples.dialTimeout,
		})
		...
		rch := cli.Watch(context.Background(), "foo", WithPrefix())
		for wresp := range rch {
			for _, ev := range wresp.Events {
				fmt.Printf("%s %q : %q\n", ev.Type, ev.Kv.Key, ev.Kv.Value)
			}
...
func ExampleWatcher_watchWithRange() {
	examples.forUnitTestsRunInMockedContext(mockWatcher_watchWithRange, func() {
		cli, err := New(Config{
			Endpoints:   examples.exampleEndpoints(),
			DialTimeout: examples.dialTimeout,
		})
		...
		rch := cli.Watch(context.Background(), "foo1", WithRange("foo4"))
		...
		for wresp := range rch {
			for _, ev := range wresp.Events {
				...
				if i == 3 {
					...
					cli.Delete(context.Background(), "foo", WithPrefix())
					cli.Close()
					return
				}
			}
...
func ExampleWatcher_watchWithProgressNotify() {
	examples.forUnitTestsRunInMockedContext(mockWatcher_watchWithProgressNotify, func() {
		cli, err := New(Config{
			Endpoints:   examples.exampleEndpoints(),
			DialTimeout: examples.dialTimeout,
		})
		...
		rch := cli.Watch(context.Background(), "foo", WithProgressNotify())
