...
func ExampleLease_grant() {
	examples.forUnitTestsRunInMockedContext(mockLease_grant, func() {
		cli, err := New(Config{
		...
		_, err = cli.Put(context.TODO(), "foo", "bar", WithLease(resp.ID))
		if err != nil {
			log.Fatal(err)
		}
	})
...
func ExampleLease_revoke() {
	examples.forUnitTestsRunInMockedContext(mockLease_revoke, func() {
		cli, err := New(Config{
			Endpoints:   examples.exampleEndpoints(),
			DialTimeout: examples.dialTimeout,
		})
...
func ExampleLease_keepAlive() {
	examples.forUnitTestsRunInMockedContext(mockLease_keepAlive, func() {
		cli, err := New(Config{
			Endpoints:   examples.exampleEndpoints(),
			DialTimeout: examples.dialTimeout,
		})
...
func ExampleLease_keepAliveOnce() {
	examples.forUnitTestsRunInMockedContext(mockLease_keepAliveOnce, func() {
		cli, err := New(Config{
			Endpoints:   examples.exampleEndpoints(),
			DialTimeout: examples.dialTimeout,
		})
