...
func ExampleMaintenance_status() {
	examples.forUnitTestsRunInMockedContext(mockMaintenance_status, func() {
		for _, ep := range examples.exampleEndpoints() {
			cli, err := New(Config{
				Endpoints:   []string{ep},
...
func ExampleMaintenance_defragment() {
	examples.forUnitTestsRunInMockedContext(mockMaintenance_defragment, func() {
		for _, ep := range examples.exampleEndpoints() {
			cli, err := New(Config{
				Endpoints:   []string{ep},
				DialTimeout: examples.dialTimeout,
