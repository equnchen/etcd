...
import (
	"context"
	"fmt"
	"go.etcd.io/etcd/tests/v3/integration/clientv3/concurrency"
	"log"
	"sync"
	"time"
...
func ExampleElection_Campaign() {
	concurrency.forUnitTestsRunInMockedContext(
		mockElectionCampaign,
		func() {
			cli, err := clientv3.New(clientv3.Config{Endpoints: concurrency.exampleEndpoints()})
			if err != nil {
				log.Fatal(err)
			}
			defer cli.Close()
			...
			s1, err := NewSession(cli)
			if err != nil {
				log.Fatal(err)
			}
			defer s1.Close()
			e1 := NewElection(s1, "/my-election/")

			s2, err := NewSession(cli)
			if err != nil {
				log.Fatal(err)
			}
			defer s2.Close()
			e2 := NewElection(s2, "/my-election/")

			// create competing candidates, with e1 initially losing to e2
			var wg sync.WaitGroup
			wg.Add(2)
			electc := make(chan *Election, 2)
			go func() {
				defer wg.Done()
